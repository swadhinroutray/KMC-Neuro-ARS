...
class FormModel {
	...
	get appointmentDefaults() {
		return {
			oneMonth: this.setDate(
				addMonths(new Date(this.dateDischarge), 1)
			),
			threeMonths: this.setDate(
				addMonths(new Date(this.dateDischarge), 3)
			),
			sixMonths: this.setDate(
				addMonths(new Date(this.dateDischarge), 6)
			),
			oneYear: this.setDate(
				addMonths(new Date(this.dateDischarge), 12)
			),
			additional: '',
		};
	}
	...
	findClosestWeekDay(selectedWeekDay, originalDate) {
		...
		let previousOccurence = getNextWeekdayOccurence(
			preferredWeekDayNumber,
			subWeeks(formattedDate, 1)
		);
		let nextOccurence = getNextWeekdayOccurence(
			preferredWeekDayNumber,
			formattedDate
		);
	...
	setDayOfWeek(weekday) {
		...
		if (this.dayOfWeek && this.dayOfWeek.trim() !== '') {
			this.dateOneMonth = this.setDate(
				this.findClosestWeekDay(
					this.dayOfWeek,
					this.appointmentDefaults['oneMonth']
				)
			);
			this.dateThreeMonths = this.setDate(
				this.findClosestWeekDay(
					this.dayOfWeek,
					this.appointmentDefaults['threeMonths']
				)
			);
			this.dateSixMonths = this.setDate(
				this.findClosestWeekDay(
					this.dayOfWeek,
					this.appointmentDefaults['sixMonths']
				)
			);
			this.dateOneYear = this.setDate(
				this.findClosestWeekDay(
					this.dayOfWeek,
					this.appointmentDefaults['oneYear']
				)
			);
		} else {
			// Reset to defaults
			this.dateOneMonth = this.appointmentDefaults['oneMonth'];
			this.dateThreeMonths =
				this.appointmentDefaults['threeMonths'];
			...
			toast.error(
				`Error: ${
					res.data
						...
						: "Couldn't add appointment record. Check all fields."
				}`,
				{
					position: 'top-right',
					autoClose: false,
