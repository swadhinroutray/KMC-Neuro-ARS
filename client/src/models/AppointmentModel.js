...
class AppointmentModel {
	...
	deleteAppointment(appointmentId) {
		...
		if (index === -1) {
			...
			toast.error(
				"Error: An error occured. Couldn't delete the appointment",
				{
					position: 'top-right',
					autoClose: 4000,
		...
		try {
			post('/api/cancel', {
				...
				appointVal:
					this._appointments[index]['appointmentType'],
	...
	fetchAppointmentsControl = (res) => {
		...
		if (res.success) {
			...
			this._appointments = res.data.flatMap(function (
				patientRecord
			) {
				let _expandedAppointments = [];
				if (
					'appointmentDate1' in patientRecord &&
					patientRecord.appointmentDate1 !== null
				) {
					_expandedAppointments.push({
						id: patientRecord.patientID.concat(
							'oneMonth'
						),
				...
				if (
					'appointmentDate3' in patientRecord &&
					patientRecord.appointmentDate3 !== null
				) {
					_expandedAppointments.push({
						id: patientRecord.patientID.concat(
							'threeMonths'
				...
				if (
					'appointmentDate6' in patientRecord &&
					patientRecord.appointmentDate6 !== null
				) {
					_expandedAppointments.push({
						id: patientRecord.patientID.concat(
							'sixMonths'
				...
				if (
					'appointmentDate12' in patientRecord &&
					patientRecord.appointmentDate12 !== null
				) {
					_expandedAppointments.push({
						id: patientRecord.patientID.concat(
							'twelveMonths'
				...
				if (
					'customAppointmentDate' in patientRecord &&
					patientRecord.customAppointmentDate !== null
				) {
					_expandedAppointments.push({
						id: patientRecord.patientID.concat(
							'custom'
			...
			toast.error(
				'Error: Could not fetch appointment information.',
				{
					position: 'top-right',
					autoClose: 4000,
					hideProgressBar: true,
